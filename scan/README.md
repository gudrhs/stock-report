...
`universe.py`는 KRX 목록 조회를 처음 시도 포함 최대 5회 수행하며, 실패 사이에
2·4·8·16초 대기합니다. `Code`, `Name`, `Market`, `Marcap`, `Close` 컬럼과
종목코드·숫자 값을 검증합니다. 양의 시가총액·가격이 있는 코스피/코스닥 종목이
합계 1,000개 미만이거나 코스피 200개·코스닥 150개 미만이면 불완전한 응답으로
판단합니다. 이 하한은 조회 품질 검사용이며 기존 종목 선정 규칙은 유지합니다.

정상 목록은 저장소 루트의 `data/universe_cache.json`에 UTC 조회 시각과 함께
원자적으로 저장합니다. 모든 조회가 실패하면 저장 후 14일 이내인 캐시를 동일하게
검증하여 사용하고 로그에 조회 시각과 경과 시간을 표시합니다. 캐시를 사용할 때
저장 시각을 갱신하지 않습니다. 캐시가 없거나 만료·손상되었으면 명확한 오류로
중단합니다. 캐시 저장만 실패하면 경고를 남기고 정상 실시간 목록으로 진행합니다.

Actions의 기존 `결과 커밋` 단계가 캐시도 `data`와 함께 저장하므로 다음 실행에서
재사용합니다. 최초 정상 실행 전에는 캐시가 없으며, 결과 커밋 전에 작업이 실패하면
그 실행에서 만든 캐시는 원격 저장소에 남지 않습니다.

저장소 루트에서 `python -m unittest discover -s tests -v`로 네트워크 없는
회귀 테스트를 실행할 수 있습니다. Actions에서도 스캔 전에 실행합니다.
