...
TB = dict(
    T_MIN=20.0,       # 10 → 20. 홈런율 1.15x → 1.37x 구간
    B_MIN=60.0,       # 55 → 60. 홈런율 1.21x → 1.35x 구간
    TAKE=30.0,        # 익절 +30% (장중). 가장 큰 지렛대였습니다
    TAKE_MIN_DAYS=0,
    HOLD_MIN=4,       # 최소 유지일 (이 날부터 매수)
    HOLD_MAX=7,       # 이 날까지가 적기 — 넘으면 '지연' 표시만 하고 매수는 계속 허용
    HR_GS_MAX=1.0,    # 홈런 판정 청선 상한  ← 매매 로그 236건 분석 결과 5.0에서 1.0으로
    GS_ENTRY=1.0,     # 진입 허용 청선 상한 (이 값을 넘으면 매수하지 않음)
    STOP_LOSS=-10.0,  # 손절선 (장중)
)

# ── 진입시 청선(G)별 실제 성과 (2026-02~08, 완결 236건)
#      G 0~1   기존 +4.84%/45.8%   T·B -0.31%/30.8%
#      G 1~2   기존 +0.98%/40.0%   T·B -3.96%/14.3%
#      G 2~5   기존 -4.10%/ 9.1%   T·B -2.00%/20.0%
#      G 5~10  기존 -3.83%/23.5%   T·B -5.49%/14.3%
#      G 10+   기존 -4.81%/18.2%   T·B -8.27%/ 4.2%
#    점수(score)는 오히려 높을수록 나빴습니다. 청선이 유일하게 단조로운 지표입니다.
GS_ENTRY = 1.0        # 기존 규칙 진입 허용 청선 상한
...
# ── 익절을 넣은 이유 (T·B 규칙, 120영업일 · 전·후반 분할 검증)
#      익절 없음   전체 -16.3%  MDD -31.8%   전반  -4.7%  후반 -23.1%
#      익절 +20    전체  +5.9%  MDD -25.8%   전반 +27.4%  후반 -16.6%
#      익절 +30    전체 +20.9%  MDD -23.7%   전반 +34.0%  후반 -13.6%
#    모든 익절 수준이 '익절 없음'보다 전·후반 양쪽에서 좋았습니다.
#    반면 진입 파라미터 최적화(648조합)는 전반 +35% → 후반 -19%로 전부 뒤집혔습니다.
#    그래서 진입은 최소한만 조이고(T20·B60), 청산 쪽에 무게를 뒀습니다.
