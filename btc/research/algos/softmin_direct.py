...
사전 등록한 명세 (조정하지 않음)
  ...
  다른 점:
    ...
    · λ = min(0.2, 0.5·P_BH / |SoftMin_BH|)  — 매월 재학습 때 한 번, 항상 보유(w=1) 정책을 같은 풀에서
      고정된 큰 표본(64배치 × 32구간)으로 평가해 계산. P_BH ≤ 0 이거나 SoftMin_BH ≥ 0 이면 λ = 0.2.
...
N_BH_BATCHES = 64               # λ 계산용 항상 보유 표본: 64배치 × 32구간 (고정)
...
class SoftminTrainer(DirectTrainer):
    ...
    def __init__(self, cfg, seed_seq):
        ...
        # 부모의 두 난수열(초기화·표본)은 그대로 두고, 항상 보유 평가용 세 번째 난수열을 따로 씀
        self.rng_bh = np.random.default_rng(np.random.SeedSequence(seed_seq).spawn(3)[2])
    ...
    def make_pool(self, datas, T_k, first_ts, warmup):
        ...
        self.P_bh, self.sm_bh = self._bh_scores(int(self.cfg.get("bh_batches", N_BH_BATCHES)))
    ...
    def _sigma_ref(self):
        ...
        for k, d in enumerate(self.datas):
            ...
            vals.append(np.log(d.o[idx + S + 1] / d.o[idx + 1]))
        return float(np.concatenate(vals).std())

    def _m_seqs(self, n, rng):
        """구간의 하루 수익 m만 (항상 보유 평가용 — 지표 불필요)"""
    ...
    def _bh_scores(self, nb):
        """항상 보유(w=1)의 P와 SoftMin — 학습과 같은 배치 모양으로 nb개 배치를 뽑아 평균 (w=1이면 g = m, 비용 0)"""
        n = int(self.cfg.get("seq_batch", 32))
        m = self._m_seqs(nb * n, self.rng_bh).reshape(nb, n, self.L)
        k = ANN / self.sigma_ref
        P = k * m.mean(axis=(1, 2))                                           # (nb,)
        z = k * m.reshape(nb, self.G, -1).mean(axis=2)                        # (nb, G)
        return float(P.mean()), float(softmin(z, self.tau, axis=1).mean())
