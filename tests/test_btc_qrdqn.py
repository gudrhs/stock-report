...
python -m unittest tests.test_btc_qrdqn -v
  ...
  · proposed_cfg = R6 + Q1 키, 같은 시드면 첫 두 층 초기값이 R6(KTrainer)와 같음
...
def _small_trainer(M=2, K=2, N=11, seed=0):
    """float64 작은 망 (입력 4, 은닉 5·4), ReLU 꺾임을 피하려고 편향 무작위"""
    cfg = q1_cfg(members=M, hidden=(5, 4), batch=6, acts=tuple(np.linspace(0, 1, K)))
...
class Targets(unittest.TestCase):
    def test_targets_match_loop(self):
        M, K, N, B = 2, 3, 11, 5
        tr, rng = _small_trainer(M, K, N, seed=2)
        tr.cfg["gamma"] = 0.967
...
class TwoStateMDP(unittest.TestCase):
    ...
    acts (0, 1), mid ½, 비용 c = 0.003 고정 → pen = κ·ln(1 − c) ≈ −0.3005
      Z(1, 1) = 0.5/(1 − γ)             (계속 보유)
      Z(1, 0) = −0.5 + γ·max(Z(1,0)', pen + Z(1,1))   (γ = 0.5 면 다음 날 보유로 전환이 나음)
      Z(0, a) = κ·(a − ½)·m + γ·max_a' [pen·|a' − a| + mean Z(1, a')]   (상태 1 은 점질량이라 투영 오차 없음)
    """

    def _true(self, gamma):
        pen = KAPPA * math.log(1 - 0.003)
        z11 = 0.5 / (1 - gamma) if gamma > 0 else 0.5
        if gamma > 0:
            stay0 = -0.5 / (1 - gamma)
            z10 = max(stay0, -0.5 + gamma * (pen + z11))
            V = {1: max(z11, pen + z10), 0: max(z10, pen + z11)}
        else:
            z10, V = -0.5, {0: 0.0, 1: 0.0}
        tau = Q.taus(11)
        qm = SC * (0.02 * tau ** 2 - 0.004)                           # m 의 분위수 (U² 는 U 의 증가함수)
        Z0 = {1: KAPPA * 0.5 * qm + gamma * V[1],
              0: KAPPA * (-0.5) * qm[::-1] + gamma * V[0]}           # a = 0 은 −½κm → 뒤집힌 분위수
        return Z0, {1: z11, 0: z10}

    def _train(self, gamma, steps=2500):
        cfg = q1_cfg(members=3, batch=128, gamma=gamma, noise=0.0, tau=0.02)
    ...
    def _check(self, gamma):
        Z = self._train(gamma)
        Z0, Z1 = self._true(gamma)
        for a in (0, 1):
            err = np.abs(Z[0, a] - Z0[a])
            spread = Z0[a][-1] - Z0[a][0]                              # ≈ 0.5·κ·SC·0.02 ≈ 20
        ...
        w = Q.cvar_weights(11, 0.5)
        for a in (0, 1):
            self.assertLess(abs(np.sort(Z[0, a]) @ w - Z0[a] @ w), 0.03 * (Z0[a][-1] - Z0[a][0]))
...
class Walk(unittest.TestCase):
    def test_proposed_cfg_is_r6_plus_q1_keys(self):
        ...
        extra = {"algo": "qrdqn", "n_quantiles": 11, "cvar_alpha": 0.5, "huber_k": 1.0}
